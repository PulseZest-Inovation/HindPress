...
const images = [
  { src: DesignGalleryImage1, title: "Acrylic Photo Frame" },
  { src: DesignGalleryImage2, title: "Packaging Solutions" },
  { src: DesignGalleryImage3, title: "Branded T-Shirts" },
  { src: DesignGalleryImage4, title: "Awards & Certificates" },
        ...
        <div className="flex justify-center items-center mb-4">
          ...
          >
            {images.map((image, index) => (
              <motion.div
                key={index}
                className="w-[280px] md:w-[300px] flex-shrink-0 relative"
                style={{ marginRight: index !== images.length - 1 ? '16px' : 0 }}
                whileHover={{ scale: 1.05 }}
                initial={{ opacity: 0, scale: 0.8 }}
                animate={{ opacity: 1, scale: 1 }}
                transition={{ delay: index * 0.1, duration: 0.20 }}
              >
                <Card className="h-full relative" sx={{ width: '100%', height: '300px' }}>
                  <CardMedia
                    component="img"
                    image={image.src}
                    alt={image.title}
